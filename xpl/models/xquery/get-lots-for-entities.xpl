...
<!--
	...
	Function: Execute an XQuery to get a list of lots associated with an entity URI
-->
<p:config xmlns:p="http://www.orbeon.com/oxf/pipeline" xmlns:oxf="http://www.orbeon.com/oxf/processors">
	...
	<p:processor name="oxf:url-generator">
		<p:input name="config">
			<config>
				<url>oxf:/apps/numishare/ui/xquery/get-lots-for-entities.xquery</url>
		...
		<p:input name="config">
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
				...
				<xsl:template match="/">
					...
					<xsl:variable name="entityURI" select="concat(/config/url, 'entity/', tokenize(doc('input:request')/request/request-uri, '/')[last()])"/>
					...
					<config>
						...
						<query>
							<xsl:variable name="collection">
								<xsl:value-of select="concat('&#x022;/db/', $collection-name, '/lots&#x022;')"/>
							</xsl:variable>
							
							<xsl:value-of select="replace(replace($xquery, 'COLLECTION', $collection), '%URI%', $entityURI)"/>
						</query>
					</config>
				</xsl:template>
			</xsl:stylesheet>
