...
foreach ($files as $file){
	if (strlen($file) > 0){
		...
		if ($dom->load($file) === FALSE){
			...
			$typeReferences = $xpath->query("descendant::lido:classification/lido:term[@lido:label='typereference']");
			
			//use type references
			if ($typeReferences->length > 0){
				$coinType = $typeReferences->item(0)->nodeValue;				
				if (preg_match("/numismatics.org\/ocre\/id/", $coinType)){
					$csv .= '"' . $objectNumber . '","';
					$csv .= $coinType;
					$csv .= '",""' . "\n";
				}			
...
>>>>>>> ff1d4ef1cf1e3e17fbd2afe37b3df28c84d1b5e9
						$file_headers = @get_headers($url);
						if ($file_headers[0] == 'HTTP/1.1 200 OK'){
							$nomismaId[] = strtoupper($num);
						} else {
