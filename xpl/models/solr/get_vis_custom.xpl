				...
				<xsl:variable name="service">
					<xsl:choose>
						<xsl:when test="string($lang)">
							<xsl:value-of select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+lang:', $lang, '+AND+', encode-for-uri($q), '+AND+', encode-for-uri($customQuery), '&amp;facet=false&amp;rows=0')"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+NOT(lang:*)+AND+', encode-for-uri($q), '+AND+', encode-for-uri($customQuery), '&amp;facet=false&amp;rows=0')"/>
						</xsl:otherwise>
					</xsl:choose>		
				</xsl:variable>
