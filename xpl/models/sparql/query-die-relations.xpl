...
<!--
	...
	Function: Query the dies that are in the opposite property (nmo:hasObverse or nmo:hasReverse) from the current die URI
-->
		...
		<p:input name="config">
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:numishare="https://github.com/ewg118/numishare"
				...
				<xsl:variable name="service">
					<xsl:value-of select="concat($sparql_endpoint, '?query=', encode-for-uri(replace(replace($query, '%STATEMENTS%', $statementsSPARQL), 'dieURI', $uri)), '&amp;output=xml')"/>					
				</xsl:variable>
