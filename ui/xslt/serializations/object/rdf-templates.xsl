...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:oa="http://www.w3.org/ns/oa#"
	xmlns:un="http://www.owl-ontologies.com/Ontology1181490123.owl#"
	xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#"
	xmlns:pelagios="http://pelagios.github.io/vocab/terms#" xmlns:gml="http://www.opengis.net/gml"
	xmlns:void="http://rdfs.org/ns/void#"
	xmlns:relations="http://pelagios.github.io/vocab/relations#"
	xmlns:nmo="http://nomisma.org/ontology#" xmlns:edm="http://www.europeana.eu/schemas/edm/"
	xmlns:svcs="http://rdfs.org/sioc/services#" xmlns:doap="http://usefulinc.com/ns/doap#"
	xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:crmsci="http://www.ics.forth.gr/isl/CRMsci/"
	xmlns:crmgeo="http://www.ics.forth.gr/isl/CRMgeo/" xmlns:tei="http://www.tei-c.org/ns/1.0"
	...
	<xsl:template match="nuds:nuds | nh:nudsHoard" mode="pelagios">
		...
		<pelagios:AnnotatedThing rdf:about="{$url}pelagios.rdf#{$id}">
			...
			<foaf:homepage
				rdf:resource="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}"/>
			<xsl:if test="string(@recordType)">
				<!-- dates -->
				<xsl:choose>
					<xsl:when test="$nudsGroup//nuds:typeDesc/nuds:date">
						<dcterms:temporal>start=<xsl:value-of
								select="number($nudsGroup//nuds:typeDesc/nuds:date/@standardDate)"
							/>; end=<xsl:value-of
								select="number($nudsGroup//nuds:typeDesc/nuds:date/@standardDate)"
							/></dcterms:temporal>
					</xsl:when>
					<xsl:when test="$nudsGroup//nuds:typeDesc/nuds:dateRange">
						<dcterms:temporal>start=<xsl:value-of
								select="number($nudsGroup//nuds:typeDesc/nuds:dateRange/nuds:fromDate/@standardDate)"
							/>; end=<xsl:value-of
								select="number($nudsGroup//nuds:typeDesc/nuds:dateRange/nuds:toDate/@standardDate)"
							/></dcterms:temporal>
					</xsl:when>
				</xsl:choose>
		...
		<xsl:for-each
			select="distinct-values($rdf//skos:closeMatch[contains(@rdf:resource, 'pleiades')]/@rdf:resource)">
			<oa:Annotation
		...
		<xsl:apply-templates
			...
			<xsl:with-param name="count"
				select="count(distinct-values($rdf//skos:closeMatch[contains(@rdf:resource, 'pleiades')]/@rdf:resource))"/>
	...
	<xsl:template match="nuds:nuds" mode="nomisma">
		<xsl:variable name="id" select="descendant::*[local-name() = 'recordId']"/>
		<xsl:variable name="objectURI" select="if (string($uri_space)) then
			concat($uri_space, $id)
			else
			concat($url, 'id/', $id)"/>
		...
		<xsl:choose>
			<xsl:when
						...
						<xsl:when test="@recordType = 'conceptual'">
							<xsl:choose>
								<xsl:when test="$collection-type = 'cointype'"
									>nmo:TypeSeriesItem</xsl:when>
								<xsl:when test="$collection-type = 'die'"
									>crm:E28_Conceptual_Object</xsl:when>
							</xsl:choose>
						</xsl:when>
				...
				<xsl:variable name="hasDefinition"
					select="boolean(nuds:descMeta/nuds:noteSet/nuds:note[@semantic = 'skos:definition'])"
					as="xs:boolean"/>
				...
				<xsl:element name="{$element}">
					...
					<xsl:choose>
						<xsl:when test="nuds:descMeta/nuds:typeDesc/nuds:typeSeries/@xlink:href">
							<dcterms:source
								rdf:resource="{nuds:descMeta/nuds:typeDesc/nuds:typeSeries/@xlink:href}"
							/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="@recordType = 'conceptual'">
						...
						<xsl:element name="{$element}">
							...
							<!-- map notes with skos properties into skos -->
							<xsl:apply-templates
								select="nuds:descMeta/nuds:noteSet/nuds:note[@semantic]"/>

							<!-- source nmo:TypeSeries, use typeSeries inherent to NUDS by default, if available -->
							<xsl:choose>
								<xsl:when
									test="nuds:descMeta/nuds:typeDesc/nuds:typeSeries/@xlink:href">
									<dcterms:source
										rdf:resource="{nuds:descMeta/nuds:typeDesc/nuds:typeSeries/@xlink:href}"
									/>
								</xsl:when>
							...
							<!-- other ids -->
							<xsl:apply-templates
								select="nuds:control/nuds:otherRecordId[string(@semantic)]"/>
					...
					<xsl:when test="@recordType = 'physical'">
						<xsl:element name="nmo:NumismaticObject">
							...
							<xsl:if test="descendant::mets:fileGrp[@USE = 'obverse']">
								<nmo:hasObverse
									rdf:resource="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}#obverse"
								/>
							</xsl:if>
							<xsl:if test="descendant::mets:fileGrp[@USE = 'reverse']">
								<nmo:hasReverse
									rdf:resource="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}#reverse"
								/>
							</xsl:if>

							<!-- look for combined images / treat the recto of the first card image the same -->
							<xsl:apply-templates
								select="nuds:digRep/mets:fileSec/mets:fileGrp[@USE = 'combined'] | nuds:digRep/mets:fileSec/mets:fileGrp[@USE = 'card'][1]/mets:fileGrp[@USE = 'recto']"/>
						...
						<xsl:if test="descendant::nuds:findspot[gml:location]">
							<xsl:variable name="coords" select="tokenize(descendant::nuds:findspot/gml:location/gml:Point/gml:pos, ' ')"/>
						...
						<xsl:apply-templates
							select="nuds:digRep/mets:fileSec/mets:fileGrp[@USE = 'card'][1]/mets:fileGrp[@USE = 'recto']/mets:file[@USE = 'iiif']">
							<xsl:with-param name="reference"
								select="nuds:digRep/mets:fileSec/mets:fileGrp[@USE = 'card'][1]/mets:fileGrp[@USE = 'recto']/mets:file[@USE = 'reference']/mets:FLocat/@xlink:href"
							/>
						</xsl:apply-templates>
					</xsl:when>
				</xsl:choose>
	...
	<xsl:template match="mets:fileSec" mode="nomisma">
		...
		<xsl:for-each select="mets:fileGrp[@USE = 'obverse' or @USE = 'reverse']">
			<xsl:variable name="side" select="@USE"/>
			<rdf:Description
				rdf:about="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}#{$side}">
				<xsl:for-each select="mets:file">
					<xsl:choose>
						<xsl:when test="@USE = 'thumbnail'">
							<foaf:thumbnail>
								<xsl:attribute name="rdf:resource">
									<xsl:choose>
										<xsl:when
											test="matches(mets:FLocat/@xlink:href, 'https?://')">
											<xsl:value-of select="mets:FLocat/@xlink:href"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of
			...
			<xsl:apply-templates select="mets:file[@USE = 'iiif']">
				<xsl:with-param name="reference"
					select="mets:file[@USE = 'reference']/mets:FLocat/@xlink:href"/>
			</xsl:apply-templates>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template match="nuds:typeDesc" mode="nomisma">
		...
		<xsl:apply-templates
			select="nuds:material[@xlink:href] | nuds:denomination[@xlink:href] | nuds:manufacture[@xlink:href] | nuds:shape[@xlink:href]"
			mode="nomisma"/>
		<xsl:apply-templates
			...
			mode="nomisma"/>
		<xsl:apply-templates
			select="nuds:date[@standardDate] | nuds:date[@notBefore and @notAfter] | nuds:dateRange[child::node()/@standardDate]"
			mode="nomisma"/>
	...
	<xsl:template match="nuds:obverse | nuds:reverse" mode="nomisma">
		<xsl:param name="id"/>
		<rdf:Description
			rdf:about="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}#{local-name()}">
					...
					<xsl:element name="nmo:{$element}">
						<rdf:Description>
							...
							<un:hasUncertainty>
								<xsl:attribute name="rdf:resource">
									<xsl:choose>
										<xsl:when test="@certainty = 'uncertain'"
											>http://nomisma.org/id/uncertain_value</xsl:when>
										<xsl:otherwise>
	...
	<xsl:template match="nuds:findspotDesc" mode="nomisma">
		...
		<xsl:choose>
			...
			<xsl:when test="nuds:findspot[gml:location]">
				<xsl:apply-templates select="nuds:findspot[gml:location]" mode="nomisma-object">
					<xsl:with-param name="objectURI" select="$objectURI"/>
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				...
				<crm:P7_took_place_at>
					<crm:E53_Place>
						<xsl:choose>
							...
							<xsl:when
								test="nuds:fallsWithin/nuds:geogname[@xlink:role = 'findspot']">
				...
				<crm:P7_took_place_at>
					<crm:E53_Place>
						...
						<xsl:if test="count($coords) = 2">
							...
							<geo:location rdf:resource="{$objectURI}#findspot"/>
							<crm:P168_place_is_defined_by rdf:resource="{$objectURI}#findspot"/>
	...
	<xsl:template match="nh:nudsHoard" mode="nomisma">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<nmo:Hoard
					rdf:about="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}">
					<rdf:type rdf:resource="http://www.w3.org/2004/02/skos/core#Concept"/>
				...
				<xsl:if test="descendant::nh:contentsDesc[nh:contents]">
					<dcmitype:Collection
						...
						<xsl:apply-templates
							select="$all-nodes/*[not(@xlink:href = preceding-sibling::*/@xlink:href)]"
							mode="nomisma"/>
					</dcmitype:Collection>
				</xsl:if>
			</xsl:otherwise>
	...
	<xsl:template match="nh:descMeta">
		...
		<xsl:if
			test="not(nh:hoardDesc/nh:deposit[nh:date or nh:dateRange]) and not(nh:hoardDesc/nh:closingDate[nh:date or nh:dateRange])">
			<xsl:call-template name="derive-closing-date"/>
		</xsl:if>

		<xsl:if test="nh:contentsDesc">
			<dcterms:tableOfContents
				rdf:resource="{if (string($uri_space)) then concat($uri_space, $id) else concat($url, 'id/', $id)}#contents"
			/>
		</xsl:if>
	</xsl:template>
		...
		<nmo:hasFindspot>
			...
			<xsl:choose>
				<xsl:when test="nh:geogname[@xlink:role = 'findspot'][@xlink:href]">
					...
					<xsl:attribute name="rdf:resource"
						select="nh:geogname[@xlink:role = 'findspot'][1]/@xlink:href"/>
				</xsl:when>
				<xsl:otherwise>
					<nmo:Find>
						...
						<crm:P7_took_place_at>
							<crm:E53_Place>
								...
								<xsl:choose>
									<xsl:when
										test="nh:fallsWithin[nh:geogname[@xlink:role = 'findspot'][@xlink:href]]">
										<xsl:apply-templates
											select="nh:fallsWithin[nh:geogname[@xlink:role = 'findspot'][@xlink:href]]"
										/>
									</xsl:when>
									<xsl:when
										test="nh:geogname[@xlink:role = 'findspot'][not(@xlink:href)]">
										<rdfs:label>
											<xsl:value-of
												select="nh:geogname[@xlink:role = 'findspot'][not(@xlink:href)]"
											/>
										</rdfs:label>
									</xsl:when>
								</xsl:choose>
		...
		<xsl:choose>
			...
			<xsl:when test="contains(@xlink:href, 'geonames.org')">
				...
				<xsl:variable name="geoURI"
					select="concat('https://sws.geonames.org/', $geonameId, '/#this')"/>
	...
	<xsl:template match="gml:location">
		...
		<geo:SpatialThing rdf:about="{$geoURI}">
			<rdf:type
				rdf:resource="http://www.ics.forth.gr/isl/CRMgeo/SP5_Geometric_Place_Expression"/>
			<crmgeo:Q9_is_expressed_in_terms_of
				rdf:resource="http://www.wikidata.org/entity/Q215848"/>

			<xsl:choose>
				<xsl:when test="gml:Point">
					<xsl:variable name="lat"
						select="normalize-space(substring-after(gml:Point/gml:coordinates, ','))"/>
					<xsl:variable name="long"
						select="normalize-space(substring-before(gml:Point/gml:coordinates, ','))"/>
	...
	<xsl:template match="nh:date | nh:fromDate | nh:toDate">
		<xsl:element
			name="nmo:{if (self::nh:fromDate) then 'hasStartDate' else if (self::nh:toDate) then 'hasEndDate' else 'hasDate'}"
