...
<p:config xmlns:p="http://www.orbeon.com/oxf/pipeline" xmlns:oxf="http://www.orbeon.com/oxf/processors">
	...
	<p:choose href="#config">
		...
		<p:when test="/config/collection_type = 'die'">
			<!-- load SPARQL query templates -->
			<p:processor name="oxf:url-generator">
				<p:input name="config">
					<config>
						<url>oxf:/apps/numishare/ui/sparql/numishareResults-count-dies.sparql</url>
						<content-type>text/plain</content-type>
						<encoding>utf-8</encoding>
					</config>
				</p:input>
			...
			<p:processor name="oxf:url-generator">
				<p:input name="config">
					<config>
						<url>oxf:/apps/numishare/ui/sparql/numishareResults-specimens-dies.sparql</url>
						<content-type>text/plain</content-type>
						<encoding>utf-8</encoding>
					</config>
				</p:input>
			...
			<p:for-each href="#identifiers" select="//identifier" root="response" id="response">
				...
				<p:processor name="oxf:unsafe-xslt">
					...
					<p:input name="config">
						<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
							<xsl:variable name="sparql_endpoint" select="doc('input:config-xml')/config/sparql_endpoint"/>
							<xsl:variable name="namedGraph" select="doc('input:config-xml')/config/die_study/namedGraph"/>
							<xsl:variable name="query" select="doc('input:query')"/>
							
							<xsl:template match="/">
								<xsl:variable name="uri" select="."/>
								<xsl:variable name="service" select="concat($sparql_endpoint, '?query=', encode-for-uri(normalize-space(replace(replace($query, '%dieURI%', $uri), '%graphURI%', $namedGraph))), '&amp;output=xml')"/>
