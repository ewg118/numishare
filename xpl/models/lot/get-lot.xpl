				...
				<xsl:template match="/">
					...
					<xsl:choose>
						...
						<xsl:when test="contains(doc('input:request')/request/request-url, 'lot/')">							
							...
							<config>
								<url>
									<xsl:value-of select="concat(/exist-config/url, $collection-name, '/lots/', $id, '.rdf')"/>
								</url>
						...
						<xsl:otherwise>							
							<xsl:variable name="id" select="doc('input:request')/request/parameters/parameter[name='lot']/value"/>
							<config>
								<url>
									<xsl:value-of select="concat(/exist-config/url, $collection-name, '/lots/', $id, '.rdf')"/>
								</url>
