...
<xsl:stylesheet xmlns:nuds="http://nomisma.org/nuds" xmlns:nh="http://nomisma.org/nudsHoard" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:skos="http://www.w3.org/2004/02/skos/core#" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:numishare="http://code.google.com/p/numishare/"
	...
	<xsl:template name="get_date_hierarchy">
		...
		<xsl:if test="number(.)">
			...
			<xsl:variable name="decade_digit"
				select="floor(number(substring($year_string, string-length($year_string) - 1, string-length($year_string))) div 10) * 10"/>
			<xsl:variable name="decade" select="
					if ($decade_digit = 0) then
						'00'
					else
						$decade_digit"/>
	...
	<xsl:template name="get_hoard_sort_fields">
		...
		<xsl:for-each select="tokenize($sort-fields, ',')">
			...
			<xsl:for-each
				select="$temp-nudsGroup/descendant::*[local-name() = $field and local-name() != 'authority'][string(.)] | $temp-nudsGroup/descendant::*[@xlink:role = $field][string(.)]">
				<xsl:sort order="ascending"/>
				<xsl:if test="position() = 1">
