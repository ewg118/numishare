...
<!--
	Get hierarchical facets (usually geography)	
-->
					...
					<xsl:choose>
						<xsl:when test="string($q)">
							<xsl:choose>
								<xsl:when test="string($lang)">
									<xsl:value-of
										select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+lang:', $lang, '+AND+', encode-for-uri($q), '&amp;facet.sort=index&amp;rows=0&amp;facet.field=', $field, '_hier&amp;fq=collection-name:', $collection-name, '+AND+', $field, '_hier:', encode-for-uri($fq), '&amp;facet.prefix=', $prefix, '&amp;link=', encode-for-uri($link))"
									/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of
										select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+NOT(lang:*)+AND+', encode-for-uri($q), '&amp;facet.sort=index&amp;rows=0&amp;facet.field=', $field, '_hier&amp;fq=collection-name:', $collection-name, '+AND+', $field, '_hier:', encode-for-uri($fq), '&amp;facet.prefix=', $prefix, '&amp;link=', encode-for-uri($link))"
									/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="string($lang)">
									<xsl:value-of
										select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+lang:', $lang, '&amp;facet.sort=index&amp;rows=0&amp;facet.field=', $field, '_hier&amp;fq=collection-name:', $collection-name, '+AND+', $field, '_hier:', encode-for-uri($fq), '&amp;facet.prefix=', $prefix, '&amp;link=', encode-for-uri($link))"
									/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of
										select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+NOT(lang:*)&amp;facet.sort=index&amp;rows=0&amp;facet.field=', $field, '_hier&amp;fq=collection-name:', $collection-name, '+AND+', $field, '_hier:', encode-for-uri($fq), '&amp;facet.prefix=', $prefix, '&amp;link=', encode-for-uri($link))"
									/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:otherwise>
