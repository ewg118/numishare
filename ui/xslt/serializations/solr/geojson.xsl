...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0">
	...
	<xsl:template match="lst">		
		...
		<xsl:for-each select="int">
			...
			<xsl:variable name="lat" select="substring-after(tokenize(@name, '\|')[3], ',')"/>
			<xsl:variable name="long" select="substring-before(tokenize(@name, '\|')[3], ',')"/>
	...
	<xsl:template match="doc" mode="hoard">
		...
		<xsl:variable name="lat" select="substring-after(tokenize(arr[@name='findspot_geo']/str[1], '\|')[3], ',')"/>
		<xsl:variable name="long" select="substring-before(tokenize(arr[@name='findspot_geo']/str[1], '\|')[3], ',')"/>
	...
	<xsl:template match="doc" mode="query">		
		<xsl:variable name="lat" select="substring-after(tokenize(arr[@name='mint_geo']/str[1], '\|')[3], ',')"/>
		<xsl:variable name="long" select="substring-before(tokenize(arr[@name='mint_geo']/str[1], '\|')[3], ',')"/>
