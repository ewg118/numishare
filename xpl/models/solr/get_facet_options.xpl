	...
	<p:processor name="oxf:unsafe-xslt">
		...
		<p:input name="config">
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:numishare="https://github.com/ewg118/numishare">
				...
				<xsl:param name="mincount" select="doc('input:request')/request/parameters/parameter[name='mincount']/value"/>				
				<!-- config variables -->
						...
						<xsl:when test="$pipeline='results'">
							<xsl:choose>
								<xsl:when test="string($lang)">
									<xsl:value-of select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+lang:', $lang, '+AND+', encode-for-uri($q), '&amp;category=', $category, '&amp;facet.field=', $category, '&amp;facet.sort=index&amp;facet.limit=-1&amp;facet.mincount=', $mincount, '&amp;rows=0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+NOT(lang:*)+AND+', encode-for-uri($q), '&amp;category=', $category, '&amp;facet.field=', $category, '&amp;facet.sort=index&amp;facet.limit=-1&amp;facet.mincount=', $mincount, '&amp;rows=0')"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="string($lang)">
									<xsl:value-of select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+lang:', $lang, '+AND+', encode-for-uri($q), '+AND+(mint_geo:*+OR+productionPlace_geo:*+OR+findspot_geo:*+OR+hoard_geo:*+OR+subject_geo:*)&amp;category=', $category, '&amp;facet.field=', $category, '&amp;facet.sort=index&amp;facet.limit=-1&amp;facet.mincount=', $mincount, '&amp;rows=0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat($solr-url, '?q=collection-name:', $collection-name, '+AND+NOT(lang:*)+AND+', encode-for-uri($q), '+AND+(mint_geo:*+OR+productionPlace_geo:*+OR+findspot_geo:*+OR+hoard_geo:*+OR+subject_geo:*)&amp;category=', $category, '&amp;facet.field=', $category, '&amp;facet.sort=index&amp;facet.limit=-1&amp;facet.mincount=', $mincount, '&amp;rows=0')"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:otherwise>
					</xsl:choose>										
