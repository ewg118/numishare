...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	exclude-result-prefixes="xs nuds nh xlink mets numishare" xmlns:geo="http://www.w3.org/2003/01/geo/wgs84_pos#"
	...
	xmlns:oa="http://www.w3.org/ns/oa#" xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:dcmitype="http://purl.org/dc/dcmitype/"
	xmlns:pelagios="http://pelagios.github.io/vocab/terms#" xmlns:edm="http://www.europeana.eu/schemas/edm/" xmlns:svcs="http://rdfs.org/sioc/services#"
	xmlns:doap="http://usefulinc.com/ns/doap#" xmlns:relations="http://pelagios.github.io/vocab/relations#" xmlns:void="http://rdfs.org/ns/void#"
	xmlns:foaf="http://xmlns.com/foaf/0.1/" xmlns:nmo="http://nomisma.org/ontology#" xmlns:gx="http://www.google.com/kml/ext/2.2"
	xmlns:mets="http://www.loc.gov/METS/" xmlns:numishare="https://github.com/ewg118/numishare" version="2.0">
			...
			<xsl:if test="string(@recordType)">
				...
				<xsl:choose>
					<xsl:when test="$nudsGroup//nuds:typeDesc/nuds:date">
						<dcterms:temporal>start=<xsl:value-of select="numishare:iso-to-digit($nudsGroup//nuds:typeDesc/nuds:date/@standardDate)"/>;
								end=<xsl:value-of select="numishare:iso-to-digit($nudsGroup//nuds:typeDesc/nuds:date/@standardDate)"/></dcterms:temporal>
					</xsl:when>
					<xsl:when test="$nudsGroup//nuds:typeDesc/nuds:dateRange">
						<dcterms:temporal>start=<xsl:value-of
								select="numishare:iso-to-digit($nudsGroup//nuds:typeDesc/nuds:dateRange/nuds:fromDate/@standardDate)"/>; end=<xsl:value-of
	...
	<xsl:template match="doc" mode="pelagios" exclude-result-prefixes="#all">
		...
		<xsl:call-template name="iiif">
			...
			<xsl:with-param name="service" select="substring-before(str[@name = 'reference_obv'], '/full')"/>
		</xsl:call-template>
		
		<xsl:call-template name="iiif">
			<xsl:with-param name="reference" select="str[@name = 'reference_rev']"/>
			<xsl:with-param name="service" select="substring-before(str[@name = 'reference_rev'], '/full')"/>
		</xsl:call-template>
