...
<!--
	...
	Function: execute a SPARQL query with an optional page parameter to get examples of physical specimens for a given coin type, limited to a value set in the config (or 48 if not set)	
-->
		...
		<p:input name="config">
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
				...
				<xsl:variable name="service">
					<xsl:value-of select="concat($sparql_endpoint, '?query=', encode-for-uri(replace(replace(replace($query, 'typeURI', $uri), '%OFFSET%', $offset), '%LIMIT%', $limit)), '&amp;output=xml')"/>					
				</xsl:variable>
