...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mets="http://www.loc.gov/METS/"
	xmlns:numishare="http://code.google.com/p/numishare/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:skos="http://www.w3.org/2004/02/skos/core#"
	...
	<xsl:template name="obverse_image">
		...
		<xsl:variable name="master_image">
			<xsl:if test="string(//mets:fileGrp[@USE='obverse']/mets:file[@USE='master']/mets:FLocat/@xlink:href)">
				<xsl:value-of select="//mets:fileGrp[@USE='obverse']/mets:file[@USE='master']/mets:FLocat/@xlink:href"/>
			</xsl:if>
		</xsl:variable>
			...
			<xsl:when test="$nuds:typeDesc/nuds:typeDesc/nuds:obverse">
				<xsl:for-each select="$nuds:typeDesc/nuds:typeDesc/nuds:obverse">
					...
					<div class="reference_image">
						...
						<xsl:text> (</xsl:text>
						<a href="{$display_path}{$master_image}">Full View</a>
						<xsl:text>)</xsl:text>
					</div>
				</xsl:for-each>
			</xsl:when>
	...
	<xsl:template name="reverse_image">
		...
		<xsl:variable name="master_image">
			<xsl:if test="string(//mets:fileGrp[@USE='reverse']/mets:file[@USE='master']/mets:FLocat/@xlink:href)">
				<xsl:value-of select="//mets:fileGrp[@USE='reverse']/mets:file[@USE='master']/mets:FLocat/@xlink:href"/>
			</xsl:if>
		</xsl:variable>
