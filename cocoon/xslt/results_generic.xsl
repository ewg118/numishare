		...
		<li class="rfl">
			...
			<xsl:choose>
				<xsl:when test="contains(@name, '_hier')">
					...
					<button class="ui-multiselect ui-widget ui-state-default ui-corner-all hierarchical-facet" type="button" title="{$title}" aria-haspopup="true" style="width: 175px;"
			...
			<xsl:when test="str[@name='recordType'] = 'conceptual'">
				<xsl:choose>
					<xsl:when test="string($sparql_endpoint)">
						<cinclude:include src="cocoon:/widget?uri={'http://numismatics.org/ocre/'}id/{str[@name='recordId']}&amp;template=results"/>
					</xsl:when>
					<xsl:otherwise>
