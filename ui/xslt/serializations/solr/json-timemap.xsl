...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" xmlns="http://earth.google.com/kml/2.0" version="2.0">
	...
	<xsl:template match="doc">
		...
		<xsl:variable name="description">
			<![CDATA[<dl class='dl-horizontal'><dt>ID</dt><dd><a href=']]><xsl:value-of select="concat($url, 'id/', str[@name='recordId'])"/>
			<![CDATA['>]]><xsl:value-of select="str[@name='recordId']"/><![CDATA[</a></dd><dt>Closing Date</dt><dd>]]><xsl:value-of select="str[@name='closing_date_display']"/>
			<![CDATA[</dd>]]>
		...
		"title": "<xsl:value-of select="$findspot"/>", <xsl:if test="number(int[@name='tpq_num'])">"start": "<xsl:value-of select="int[@name='tpq_num']"/>",</xsl:if>
		<xsl:if test="number(int[@name='taq_num'])">"end": "<xsl:value-of select="int[@name='taq_num']"/>",</xsl:if> "options": { "theme": "<xsl:value-of select="red"/>"<xsl:if
