...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:nh="http://nomisma.org/nudsHoard" xmlns:nuds="http://nomisma.org/nuds"
    xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:numishare="https://github.com/ewg118/numishare"
    xmlns:nomisma="http://nomisma.org/" xmlns:skos="http://www.w3.org/2004/02/skos/core#"
    ...
    <xsl:function name="numishare:recompile_category">
        ...
        <xsl:value-of
            select="substring-after(replace($tokenized_fragment[contains(., concat('L', $level, '|'))], '&#x022;', ''), '|')"/>
        <xsl:if test="contains($category_fragment, concat('L', $level + 1, '|'))">
            <xsl:text>--</xsl:text>
            <xsl:value-of
                select="numishare:recompile_category($tokenized_fragment, $category_fragment, $level + 1)"
            />
        </xsl:if>
    </xsl:function>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$lang = 'ar'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'collection'">دار الكتب و الوثائق القومية
                        المصرية</xsl:when>
            ...
            <xsl:when test="$lang = 'ro'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'landowner'">Proprietarul locului de
                        descoperire</xsl:when>
                    ...
                    <xsl:when test="$label = 'timestamp'">Data modificării de intrare în
                        catalog</xsl:when>
            ...
            <xsl:when test="$lang = 'it'">
                <xsl:choose>
                    <xsl:when test="$label = 'acknowledgment'">Ringraziamenti</xsl:when>
                    ...
                    <xsl:when test="$label = 'appraiser'">Esperto/Perito</xsl:when>
                    ...
                    <xsl:when test="$label = 'date'">Data</xsl:when>
                    <xsl:when test="$label = 'method'">Metodo di indagine</xsl:when>
                    <xsl:when test="$label = 'period'">Periodizzazione</xsl:when>
                    ...
                    <xsl:when test="$label = 'denomination'">Nominale</xsl:when>
                    <xsl:when test="$label = 'department'">Dipartimento/Ufficio</xsl:when>
                    ...
                    <xsl:when test="$label = 'diameter'">Diametro (in mm)</xsl:when>
                    ...
                    <xsl:when test="$label = 'previousColl'">Collezione/collocazione precedente</xsl:when>
                    ...
                    <xsl:when test="$label = 'maker'">Produttore</xsl:when>
                    <xsl:when test="$label = 'artist'">Artista</xsl:when>
                    <xsl:when test="$label = 'engraver'">Incisore</xsl:when>
                    <xsl:when test="$label = 'license'">Licenza d'uso</xsl:when>
                    <xsl:when test="$label = 'rights'">Diritti</xsl:when>
                    <xsl:when test="$label = 'location'">Posizione</xsl:when>
                    <xsl:when test="$label = 'rightsStmt'">Diritti</xsl:when>
                    <xsl:when test="$label = 'latitude'">Latitudine</xsl:when>
                    <xsl:when test="$label = 'longitude'">Longitudine</xsl:when>
                    ...
                    <xsl:when test="$label = 'testmark'">Punzonatura/Saggiatura</xsl:when>
                    ...
                    <xsl:when test="$label = 'thickness'">Spessore (in mm)</xsl:when>
                    <xsl:when test="$label = 'undertypeDesc'">Descrizione del sottotipo</xsl:when>
                    <xsl:when test="$label = 'wear'">Leggibilità</xsl:when>
                    <xsl:when test="$label = 'weight'">Peso (in g)</xsl:when>
            ...
            <xsl:when test="$lang = 'fi'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'dateOnObject'">esineessä esiintyvä
                        päivämäärä</xsl:when>
            ...
            <xsl:when test="$lang = 'el'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'dateOnObject'">Χρονολογία επί του
                        αντικειμένου</xsl:when>
                    ...
                    <xsl:when test="$label = 'timestamp'">Ημερομηνία τροποποίησης
                        εγγραφής</xsl:when>
            ...
            <xsl:when test="$lang = 'es'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'timestamp'">Fecha de modificación del
                        registro</xsl:when>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'authorizingEntity'">Political/Dynastic
                        Entity</xsl:when>
                    ...
                    <xsl:when test="$label = 'custodian'">Body in charge of excavation</xsl:when>
                    <xsl:when test="$label = 'date'">Date</xsl:when>
                    ...
                    <xsl:when test="$label = 'depositDate'">Date of Deposit</xsl:when>
                    <xsl:when test="$label = 'depth'">Depth</xsl:when>
                    <xsl:when test="$label = 'description'">Description</xsl:when>
                    <xsl:when test="$label = 'diameter'">Diameter (in mm)</xsl:when>
                    ...
                    <xsl:when test="$label = 'rightsStmt'">Rights</xsl:when>
                    ...
                    <xsl:when test="$label = 'tombNumber'">Tomb Number</xsl:when>
                    <xsl:when test="$label = 'type'">Type</xsl:when>
        ...
        <xsl:choose>
            <xsl:when test="contains($field, 'legendCondensed')">
                <xsl:variable name="label"
                    select="substring-before(replace($field, 'legendCondensed', 'leg'), '_text')"/>
        ...
        <xsl:choose>
            <xsl:when test="$lang = 'ur'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'header_analyze'">سکوں کے مجموعہ جات کا
                        تجزیہ</xsl:when>
                    <xsl:when test="$label = 'header_visualize'">سوالات کا تجزیہ کرنا</xsl:when>
                    <xsl:when test="$label = 'header_contributors'">معاونین/ حصہ ڈالنے
                        واے</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">کل نتائج ZZ میں سے، XX سے YY تک
                        دکھائے جارہے ہیں</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">منتخب کردہ شرائط کی بنیاد پر چارٹ
                        تخلیق کرنے کے لئے ذیل میں دئیے گئے کوائف کا انتخاب اور تصوری اختیارات کا
                        استعمال کریں۔ اس خصوصیت کو استعمال کرنے کے لئے ہدایات یہاں مل سکتی
                        ہیں-</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">مندرجہ ذیل قسموں کی فیصد یا عددی
                        تعدار کو دیکھنے کے لئے اس فیچر کا استعمال کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">اس خصوصیت کا استعمال سکوں کے
                        مجموعہ جات کے اندر پائے جانے والے، کسی خاص تاریخ کے سکوں، کی فیصد یا تعداد
                        کو جاننے کے لئے کریں۔</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">دیئے گئے استفسار(سوال) اور منتخب
                        کردہ سکوں کے مجموعہ جات کیلئے CSV منتقل کرنے کیلئے اس خصوصیت کا استعمال
                        کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">پیمائش منتخب
                        کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">نقشہ کی قسم منتخب
                        کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">تجزیہ کے لئے زمرے منتخب
                        کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">سکوں کے مجموعہ جات منتخب
                        کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">موازنہ کرنے کے لیے
                        سکوں کے مجموعہ جات کا انتحاب کریں (اختیاری)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">زمرہ (قسم) کے لحاظ سے
                        موازنہ کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">دیگر سوالات سے موازنہ
                        کریں (اختیاری)</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_custom'">مخصوص سوالات شامل
                        کریں</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_remove_filter'">چھانٹی کو ختم
                        کریں</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">اپنی پیمائش کے سوال کو
                        ترتیب دینے کے لئے نیچے والی فہرست کا استعمال کریں۔ تاریخ کی حد ہرسوال میں
                        صرف ایک بار متعین کی جاسکتی ہے۔</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">اختیاری
                        ترتیبات</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">چھپائیں / دکھائیں
                        اختیارات</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">ڈھیر لگانے کے
                        اختیارات</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">یقینی ضابطوں کو
                        شامل نہ کریں</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_calculate'">منتخب شدہ کا حساب
                        لگائیں</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">مجموعی فی
                        صد</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">وقفے اور دورانیے کی ضرورت ہے
                        ۔</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">تا تاریخ، از تاریخ کے بعد کی ہونی
                        ضروری ہے۔</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">ایک زمرے کا انتخاب کرنا
                        ہوگا۔</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">کم از کم 1 اور زیادہ سے زیادہ 8
                        سکوں کے مجموعہ جات کا انتخاب ہوسکتا ہے۔</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">زیادہ سے زیادہ 8 سکوں کے مجموعہ جات
                        کا انتخاب ہوسکتا ہے۔</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">کم از کم 1 اور زیادہ سے زیادہ 30
                        سکوں کے مجموعہ جات کا انتخاب ہوسکتا ہے۔</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">زیادہ سے زیادہ 30 سکوں کے مجموعہ
                        جات کا انتخاب ہوسکتا ہے۔</xsl:when>
            ...
            <xsl:when test="$lang = 'de'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_subtype_desc'">Ein Klick auf den Link zum
                        Untertyp zeigt alle Exemplare.</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-results'">Ergebnisse
                        eingrenzen</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-category'"
                        >Sortierungskategorie</xsl:when>
                    <xsl:when test="$label = 'results_ascending'">Aufsteigend</xsl:when>
                    <xsl:when test="$label = 'results_descending'">Absteigend</xsl:when>
                    <xsl:when test="$label = 'results_result-desc'">Ergebnisse von XX bis YY aus ZZ
                        Gesamtergebnissen anzeigen</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_typological'">Typologische
                        Analyse</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">Analyse der
                        Dimensionen</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Verwenden Sie die Datenauswahl- und
                        Visualisierungsoptionen, um eine auf ausgewählten Parametern basierte Grafik
                        zu erzeugen. Eine Gebrauchsanleitung dafür finden Sie hier</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Verwenden Sie diese Option, um
                        Prozent- bzw. absolute Zahlen der folgenden Typologien zu
                        visualisieren</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Verwenden Sie diese Option, um
                        das Vorkommen von Münzen eines bestimmten Datums in Schtazfunden als
                        Prozent- bzw. absolute Zahlen zu erhalten</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Verwenden Sie diese Option, um
                        eine CSV-Datei für die Abfrage und die ausgewählten Schatzfunde zu
                        downloaden</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Dimensionen
                        auswählen</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Grafikart auswählen</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Analysekategorien
                        auswählen</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Schatzfunde
                        auswählen</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Schatzfunde für
                        Verleich auswählen (optional)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Vergleich nach
                        Kategorie</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Vergleich mit anderen
                        Abfragen (optional)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Abfragen vergleichen</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'"
                        >Vergleichsabfrage</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Spezifische Abfrage
                        hinzufügen</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Spezifische
                        Abfrage</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Verwenden Sie die
                        Ausklappmenüs unten, um Ihre Dimensionsabfrage zu formulieren. Ein
                        Datumsbereich kann für jede Abfrage nur einmal angegeben werden</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Optionale
                        Einstellungen</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Optionen
                        ausblenden/zeigen</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Stapeloptionen</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Sicherheitscodes
                        entfernen</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Nach Intervall anordnen
                        (optional)</xsl:when>
                    <xsl:when test="$label = 'visualize_interval'">Intervall (Jahre)</xsl:when>
                    <xsl:when test="$label = 'visualize_duration'">Dauer</xsl:when>
                    <xsl:when test="$label = 'visualize_calculate'">Ausgewähltes
                        berechnen</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Cumulative
                        Prozentzahl</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Intervall und Dauer werden
                        benötigt</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Datum bis muss später als Datum von
                        sein</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Eine Kategorie muss ausgewählt
                        sein</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Mindestens 1 und bis zu 8
                        Schatzfunden können ausgewählt werden</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Bis zu 8 Schatzfunden können
                        ausgewählt werden</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Mindestens 1 und bis zu 30
                        Schatzfunden können ausgewählt werden</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Bis zu 30 Schatzfunden können
                        ausgewählt werden</xsl:when>
            ...
            <xsl:when test="$lang = 'ar'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'header_visualize'">التصور المرئى
                        للإستفسارات</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">عرض سجلات من (أ أ) إلى (ب ب) من
                        إجمالى النتائج (ج ج) بالكامل</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_typological'">تحليل الخواص الهندسية و
                        الفراغية</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">تحليل القياسات</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">استخدم الخيارات و التصور المرئى
                        للبيانات أدناه لتوليد مخطط بيانى استنادا إلى هذه المعايير. تعليمات إستخدام
                        هذه الخاصية يمكن العثور عليها هنا</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">استخدم هذه الخاصية لإظهار التصور
                        البصرى للنسب المئوية أو الظهور العددى للنماذج التالية</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">إستخدم هذه الخاصية لتوليد النسب
                        المئوية أو الظهور العددى للقطع النقدية من تاريخ معين ضمن الكنوز</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">إستخدم هذه الخاصية لتحميل
                        سى-إس-ڤى للبحث المُقَدَّم و الكنوز المختارة</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">إختر المقاس</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">حدد نوع الرسم
                        البياني</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">إختر الكنوز
                        للمقارنة، إستخدام بين القوسين اختيارى</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">مقارنة حسب
                        الفئة</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">مقارنة مع الإستفسارات
                        الأخرى، إستخدام بين القوسين إختيارى</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">مقارنة بين الإستفسارات</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">إستفسار
                        للمقارنة</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">إضافة بحث/إستفسار
                        مخصص</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_query'">أضِف عوامل تصفية
                        للإستفسارات</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">إزالة عوامل
                        التصفية</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">إستخدم القوائم المنسدلة
                        أدناه لصياغة الإستعلام الخاص بك. لا يمكن تحديد نطاق التاريخ أكثر مرة واحدة
                        في الإستعلام</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">محدِدات
                        اختيارية</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">خيارات الإخفاء أو
                        العرض</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">تراص الخيارات</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">إستبعاد الأكواد
                        اليقينية</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">رَتِّب حسب الفترات الزمنية -
                        إختيارى</xsl:when>
                    <xsl:when test="$label = 'visualize_interval'">رَتِّب على فترات زمنية متساوية،
                        بالعام</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">نسبة مئوية
                        تراكمية</xsl:when>
                    ...
                    <xsl:when test="$label = 'chart_areaspline'">المنطقة المحصورة تحت المنحنى
                        البيانى أو بين منحنيين بيانيين</xsl:when>
                    <xsl:when test="$label = 'chart_line'">خط الرسم البيانى</xsl:when>
                    <xsl:when test="$label = 'visualize_alert'">تنبيه</xsl:when>
                    <xsl:when test="$label = 'visualize_error1'">مطلوب الفترات المتساوية و
                        مدتها</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">تاريخ نهاية المدة يجب أن يكون
                        لاحقاً لتاريخ بدايتها</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">يجب إختيار الفئة/النوع</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">يجب إختيار كنز واحد على الأقل، و
                        يمكن حتى ثمانية</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">يمكن أختيار ما يصل إلى ثمانية
                        كنو</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">يجب إختيار كنز واحد على الأقل، و
                        يمكن حتى ثلاثين</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">يمكن أختيار ما يصل إلى ثلاثين
                        كنز</xsl:when>
            ...
            <xsl:when test="$lang = 'fr'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_data-download'">Récupérer les
                        données</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-search'">Raffiner la
                        recherche</xsl:when>
                    <xsl:when test="$label = 'results_select'">Sélectionner à partir de la
                        liste</xsl:when>
                    <xsl:when test="$label = 'results_sort-results'">Classer les
                        résultats</xsl:when>
                    <xsl:when test="$label = 'results_sort-category'">Classer les
                        catégories</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Afficher les références XX à YY
                        à partir de ZZ résultats</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Utilisez la sélection des
                        informations et les options de visualisation ci-dessous pour créer un
                        graphique basés sur les paramètres sélectionnés. Les instructions
                        d'utilisation de cette fonction peuvent être trouvés ici.</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Utilisez cette fonctionalité
                        pour visualiser les pourcentages ou le nombre de fois où cette typologie
                        apparaît</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Utilisez cette fonctionalité
                        pour obtenir le pourcentage ou le nombre de fois où cette monnaie apparaît à
                        une date particulière au sein des trésors</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Utilisez cette fonctionalité pour
                        télécharger un CSV correspondant à la recherche et les trésors
                        sélectionnés</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Selectionnez
                        Mesure</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Sélectionnez Type de
                        graphe.</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Sélectionnez Catégories pour
                        analyse.</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Sélectionnez
                        Trésors</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Sélectionnez
                        Comparaison de trésors (Optionnel)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Comparez par
                        catégorie</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Comparez aux autres
                        recherches (optionnel)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Comparez les recherches</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Demande de
                        comparaison</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Ajoutez des demandes
                        spécifiques</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_queries'">Ajoutez des
                        demandes</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Utilisez les fonctions de
                        défilement ci-dessous pour fromuler votre demande de mesure. Une période
                        entre deux dates ne peut être spécifiée qu'une seule fois par
                        demande</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Cadre de travail
                        optionnel</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Options de
                        Cacher/Afficher</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Options de mise en
                        ordre</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Exclure les codes
                        certains</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Organiser par intervalles
                        (optionnel)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Pourcentage
                        cumulé</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Intervalle et période sont
                        requis</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">La date finale doit se situer après
                        la date initiale</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Une catégorie doit être
                        sélectionnée</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Un minimum de 1 et un maximum de 8
                        trésors peuvent être sélectionnés</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Jusqu'à 8 trésors peuvent être
                        sélectionnés</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Un minimum de 1 et un maximum de 30
                        trésors peuvent être sélectionnés</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Jusqu'à 30 trésors peuvent être
                        sélectionnés</xsl:when>
            ...
            <xsl:when test="$lang = 'ro'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'results_map-results'">Rezultatele
                        geografice</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_data-options'">Opţiuni asupra
                        datelor</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-results'">Clasificarea
                        rezultatelor</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Afişare rezultate de la XX la YY
                        din totalul de ZZ rezultate</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Utilizați selecția de date și
                        opțiunile de vizualizare de mai jos pentru a genera harta pe baza
                        parametrilor selectați. Instrucțiuni de utilizare pentru această funcție pot
                        fi găsite aici.</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Utilizați această funcție pentru
                        a vizualiza procentajul sau cuantificarea următoarelor tipologii</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Utilizați această funcție pentru
                        a reda procentajul sau cuantificarea monedelor emise la o anumită dată din
                        cadrul tezaurelor</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Utilizați această funcție pentru
                        a descărca CSV (valori separate prin virgulă) pentru cererea adresată și
                        tezaurele selectate</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Selectare
                        dimensiuni</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Selectare tip de
                        hartă</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Selectare Categorii pentru
                        Analiză</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Selectare Tezaure</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Selectare Tezaure
                        pentru Comparație (opțional)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Comparație pe
                        Categorie</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Comparație cu alte Cereri
                        (opțional)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Comparare Categorii</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Categorie de
                        Comparație</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Adăugare Categorie
                        aleatorie</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Categorie
                        aleatorie</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_query'">Eliminare
                        categorie</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Utilizați meniul drop-down
                        pentru a fomula criteriul de măsurare. Intervalul cronologic poate fi
                        speificat doar o dată pe cerere</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Setări
                        opționale</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Ascunde/arată opțiuni</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Pachet de
                        opțiuni</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Excludere coduri
                        de siguranță</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Aranjare pe segmente
                        (opțional)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative'">Cumulativ</xsl:when>
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Procentaj
                        cumulativ</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Interval și perioadă sunt
                        necesare</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Datarea până la...trebuie să fie
                        mai târziu decât Datare de la...</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Trebuie să fie selectată o
                        categorie</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Pot fi selectate de la cel puțin 1
                        tezaur până la maxim 8</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Pot fi selectate maxim 8
                        tezaure</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Pot fi selectate de la cel puțin 1
                        tezaur până la maxim 30</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Pot fi selectate maxim 30
                        tezaure</xsl:when>
            ...
            <xsl:when test="$lang = 'ru'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_quantitative'">Количественный
                        анализ</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_map-results'">Показать результаты на
                        карте</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-results'">Обновить
                        результаты</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-results'">Сортировать
                        результаты</xsl:when>
                    <xsl:when test="$label = 'results_sort-category'">Сортировать
                        категории</xsl:when>
                    <xsl:when test="$label = 'results_ascending'">По возрастанию</xsl:when>
                    <xsl:when test="$label = 'results_descending'">По убыванию</xsl:when>
                    <xsl:when test="$label = 'results_result-desc'">Отображать записи XX из YY из ZZ
                        результатов</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_typological'">Типологический
                        анализ</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">Анализ измерений</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Использовать выбор данных и
                        визуализацию параметров дальше для создания диаграммы на основе выбранных
                        параметров. Инструкции по использованию этой функции можно найти
                        здесь</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Использовать эту функцию, чтобы
                        визуализировать проценты или числовые совпадения имеющихся
                        типологий</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Использовать функцию, чтобы
                        рассчитать проценты или количество монет определенной даты в рамках
                        кладов</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Использовать функцию для загрузки
                        CSV для данного запроса и выбранных кладов</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Выбрать
                        измерения</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Тип диаграммы</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Выбрать категории для
                        анализа</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Выбрать клады</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Выбрать клады для
                        сравнения (по желанию)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Сравнить
                        категории</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Сравнить с другими
                        запросами (по желанию)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Сравнить запросы</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Сравнение
                        запроса</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Добавить пользовательский
                        запрос</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Пользовательский
                        запрос</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_list'">Фильтрация по
                        списку</xsl:when>
                    <xsl:when test="$label = 'visualize_filter_query'">Фильтрация по
                        запросу</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Удалить фильтр</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Добавить описание
                        запроса</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Дополнительные
                        настройки</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Прятать/отображать
                        параметры</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Параметры
                        стека</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Исключить защитные
                        коды</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Разместить по интервалам (по
                        желанию)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Суммарный
                        процент</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Необходимы интервал и
                        продолжительность</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">&quot;К дате&quot; должно быть
                        позднее чем &quot;От даты&quot;</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Необходимо выбрать
                        категорию</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Можно выбрать от 1 до 8
                        кладов</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Можно выбрать более 8
                        кладов</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Можно выбрать от 1 до 30
                        кладов</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Можно выбрать более 30
                        кладов</xsl:when>
            ...
            <xsl:when test="$lang = 'uk'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'results_map-results'">Висвітлити результати на
                        мапі</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Відображати записи ХХ з YY з ZZ
                        результатів</xsl:when>
                    <xsl:when test="$label = 'results_coin'">монета</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Використати вибір даних і
                        візуалізацію параметрів далі для створення діаграми на основі вибраних
                        параметрів. Інструкції з використання цієї функції можна знайти
                        тут</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Використати функцію, щоб
                        візуалізувати відсотки або числові збіги наступних типологій</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Використати функцію, щоб
                        вирахувати відсотки або кількість монет певної дати в межах
                        скарбів</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Використати функцію для
                        завантаження CSV для даного запиту та обраних скарбів</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Обрати
                        вимірювання</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Тип діаграми</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Обрати категорії для
                        аналізу</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Обрати скарби</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Обрати скарби для
                        порівняння (за бажанням)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Порівняти
                        категорії</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Порівняти з іншими
                        запитами (за бажанням)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Порівняти запити</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Порівняння
                        запита</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Додати користувальницкий
                        запит</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Користувальницький
                        запит</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_list'">Фільтрація за
                        списком</xsl:when>
                    <xsl:when test="$label = 'visualize_filter_query'">Фільтрація за
                        запитом</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Видалити фільтр</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Додати опис
                        запиту</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Додаткові
                        налаштування</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Ховати/відображати
                        параметри</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Параметри
                        стека</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Виключити захісні
                        коди</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Розташувати по інтервалах (за
                        бажанням)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Сукупний
                        відсоток</xsl:when>
                    <xsl:when test="$label = 'chart_bar'">Бар</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Потрібні інтервал та
                        тривалість</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">&quot;До дати&quot; повинно бути
                        пізніше за &quot;Від дати&quot;</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Потрібно обрати
                        категорію</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Можна обрати від 1 до 8
                        скарбів</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Можна обрати більше 8
                        скарбів</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Можна обрати від 1 до 30
                        скарбів</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Можна обрати більше 30
                        скарбів</xsl:when>
            ...
            <xsl:when test="$lang = 'nl'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_quantitative'">Kwantitatieve
                        analyse</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Toon records XX tot YY van ZZ
                        resultaten.</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_typological'">Typologische
                        analyse</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">Analyse van de
                        afmetingen</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Gebruik de geselecteerde data en
                        visualiseringsopties benedenaan, om een kaart te genereren op basis van de
                        geselecteerde parameters. Een toelichting om deze functie te gebruiken vindt
                        u hier</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Hiermee worden percentages of
                        absolute aantallen van de volgende typologieën getoond</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Hiermee wordt de aanwezigheid
                        van munten van een specifiek jaar in schatvondsten in percentages of
                        absolute aantallen getoond</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Hiermee kunt u een CSV downloaden
                        met de door u geselecteerde data en geselecteerde schatvondsten</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Selecteer
                        formaat</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Selecteer kaartsoort</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Selecteer
                        analysecategorieën</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Selecteer
                        schatvondsten</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Selecteer
                        schatvondsten om te vergelijken (optioneel)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Vergelijk per
                        categorie</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Vergelijk met andere
                        zoekopdrachten (optioneel)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Vergelijk
                        zoekopdrachten</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Vergelijk
                        zoekopdracht</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Specifieke zoekopdracht
                        toevoegen</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Gedetailleerde
                        zoekopdracht</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_query'">Zoekopdracht
                        verwijderen</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query'">Zoekopdracht
                        toevoegen</xsl:when>
                    <xsl:when test="$label = 'visualize_add_queries'">Zoekopdrachten
                        toevoegen</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_query'">Filter
                        zoekopdracht</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Verwijder filter</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Gebruik de drop-down menu's
                        hieronder om de zoekvraag op basis van afmetingen samen te stellen. Per
                        zoekvraag kan alleen één datumbereik worden opgegeven</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Optionele
                        instellingen</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Opties
                        tonen/verbergen</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Stapel opties</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Verwijder
                        veiligheidscodes</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">sorteer per interval
                        (optioneel)</xsl:when>
                    <xsl:when test="$label = 'visualize_interval'">Interval (jaren)</xsl:when>
                    <xsl:when test="$label = 'visualize_duration'">Duur</xsl:when>
                    <xsl:when test="$label = 'visualize_calculate'">Bereken het
                        geselecteerde</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Cumulatief
                        percentage</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_alert'">Alarm</xsl:when>
                    <xsl:when test="$label = 'visualize_error1'">Interval en tijd zijn
                        vereist</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Einddatum moet na begindatum
                        liggen</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Selecteer een categorie</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Minimaal 1 van de 8 vondsten moeten
                        zijn geselecteerd</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Maximaal 8 vondsten kunnen worden
                        geselecteerd</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Selecteer minimaal 1 en maximaal 30
                        vondsten</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Er kunnen maximaal 30 vondsten
                        worden geselecteerd</xsl:when>
                    <xsl:when test="$label = 'lang_ar'">Arabisch</xsl:when>
            ...
            <xsl:when test="$lang = 'sv'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'results_refine-results'">Filtrera
                        resultaten</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Visa poster XX till YY från ZZ
                        resultater</xsl:when>
            ...
            <xsl:when test="$lang = 'pl'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'results_filters'">filtruj wyniki
                        wyszukiwania</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-results'">zawężone wyniki
                        wyszukiwania</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-results'">sortuj wyniki
                        wyszukiwania</xsl:when>
                    <xsl:when test="$label = 'results_sort-category'">sortuj kategorie
                        wyszukiwania</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">pokazuje rekordy od XX do YY z
                        wszystkich wyników ZZ</xsl:when>
                    ...
                    <xsl:when test="$label = 'maps_legend'">legenda</xsl:when>
                    <xsl:when test="$label = 'visualize_typological'">analiza
                        typologiczna</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">analiza
                        metrologiczna</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Użyj opcji wybierania i wizualizacji
                        danych poniżej aby uzyskać wykres oparty na wybranych parametrach.
                        Instrukcje korzystania z tej opcji znajdują się tu</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Użyj tej opcji aby wizualizować
                        procentowe lub liczbowe proporcje dla następujących typów</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Użyj tej opcji aby przedstawić
                        procentowe lub liczbowe proporocje monet o określonym datowaniu w
                        skarbach</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Użyj tej opcji aby ściągnąć
                        dokument CSV dla danej kwerendy i wybranych skarbów</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">wybierz
                        wymiary</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">wybierz typ wykresu</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">wybierz kategorie do
                        analizy</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">wybierz skarby</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">wybierz skarby do
                        porównania (opcjonalnie)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">porównaj
                        kategoriami</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">porównaj z innymi
                        kwerendami (opcjonalnie)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">porównaj kwerendy</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">kwerenda
                        porównawcza</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">dodaj kwerendę
                        niestandardową</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">kwerenda
                        niestandardowa</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_list'">filtruj wyniki na
                        liście</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Użyj list rozwijanych
                        poniżej aby sformułować swoją kwerendę dotyczącą wymiarów. Zakres dat może
                        być tylko raz określony</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">ustawienia
                        opcjonalne</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">z wyłączeniem
                        kodów trafności</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">sumarycznie
                        procentowo</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Interwał i czas trwania
                        wymagane.</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Data do nie może być wcześniejsza
                        niż data od</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Należy wybrać kategorię.</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Wybrać można co najmniej 1 i nie
                        więcej niż 8 skarbów.</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Wybrać można do 8
                        skarbów.</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Wybrać można co najmniej 1 i nie
                        więcej niż 30 skarbów.</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Wybrać można do 30
                        skarbów.</xsl:when>
            ...
            <xsl:when test="$lang = 'tr'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">XX'ten YY'ye ZZ'nin tüm
                        Sonuçları Gösteriliyor</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Seçilen Parametrelere Dayalı bir
                        Grafik Oluşturmak için Aşağıdaki Veri Seçimi ve Görselleştirme Seçenekleri
                        Kullanın. Bu Özelliği Kullanmak için Talimatları Burada
                        Bulabilirsiniz</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Takip eden Tipolojilerdeki
                        Yüzdelik veya Sayısal Oluşumları Görselleştirmek İçin bu Özelliği
                        Kullanın</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Defineler İçindeki Belirli bir
                        Tarihteki Yüzdelik veya Sayısal Oluşumları İşlemek İçin bu Özelliği
                        Kullanın</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Yapılan Sorgu ve Seçilen
                        Defineleri CSV Olarak İndirmek İçin bu Özelliği Seçin</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_categories'">Analiz İçin Kategorileri
                        Seç</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Defineleri Seç</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Karşılaştırma İçin
                        Defineleri Seç (İsteğe Bağlı)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Kategoriye göre
                        Karşılaştır</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Diğer Sorgularla
                        Karşılaştır (isteğe bağlı)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Sorguları Karşılaştır</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Karşılaştırma
                        Sorgusu</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Ölçüm sorgusunu formüle
                        etmek için açılır menüleri kullanın. Bir tarih aralığı sorgu başına sadece
                        bir kez belirtilebilir.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">İsteğe bağlı
                        seçenekeler</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Seçenekleri
                        gizle/göster</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Depolama
                        seçenekleri</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Kesinlik kodunu
                        kaldır</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Aralığı düzenleyin (isteğe
                        bağlı)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Kümilatif
                        Yüzde</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_alert'">Alarm</xsl:when>
                    <xsl:when test="$label = 'visualize_error1'">Aralık ve süre
                        gereklidir.</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">...-e kadar tarihi, ...-den
                        tarihinden daha sonra olmalıdır.</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Bir kategori seçilmeli.</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">En az 1 en fazla 8 define
                        seçilebilir.</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">8 defineye kadar
                        seçilebilir.</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">En az 1 en fazla 30 define
                        seçilebilir.</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">30 defineye kadar
                        seçilebilir.</xsl:when>
            ...
            <xsl:when test="$lang = 'fi'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'header_about'">Tietoa....-sta (e.g. tietoa
                        Numisma.orgista)</xsl:when>
                    ...
                    <xsl:when test="$label = 'header_analyze'">analysoi rahakätköjä/ tutki
                        rahakätköjä</xsl:when>
                    ...
                    <xsl:when test="$label = 'display_administrative'"
                        >hallinto/hallinnallinen</xsl:when>
                    <xsl:when test="$label = 'display_visualization'"
                        >visualisointi/havainnollistaminen</xsl:when>
                    <xsl:when test="$label = 'display_data-download'">tietojen lataus</xsl:when>
                    <xsl:when test="$label = 'display_quantitative'">kvantitatiivinen
                        analyysi/kvantitatiivinen tutkimus</xsl:when>
                    ...
                    <xsl:when test="$label = 'display_examples'">Esimerkkejä tämäntyyppisistä
                        esineistä/ esimerkkejä tälläisesta</xsl:when>
                    <xsl:when test="$label = 'results_all-terms'">kaikki termit/ kaikki
                        hakusanat</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_data-options'">datan vaihtoehdot/tietojen
                        vaihtoehdot</xsl:when>
                    <xsl:when test="$label = 'results_refine-results'">tarkenna
                        halutuloksia</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-results'">lajittele tulokset/järjestele
                        tulokset</xsl:when>
                    <xsl:when test="$label = 'results_sort-category'">lajittele kategoria/ lajittele
                        luokka</xsl:when>
                    <xsl:when test="$label = 'results_ascending'">nouseva</xsl:when>
                    <xsl:when test="$label = 'results_descending'">laskeva</xsl:when>
                    <xsl:when test="$label = 'results_result-desc'">Näyttää merkinnät XX - YY ZZ:sta
                        (e.g. 5-10 100:sta)</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_typological'">typologinen
                        analyysi/typologinen tutkimus</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">mittauksen analyysi/
                        mittauksen tutkimus</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Voit käytää alla olevia vaihtoehtoja
                        luodaksesi valittujen rajoituksien tuottaman diagrammin. Neuvoja löydät
                        täältä</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Havainnollista näiden
                        typologioiden ilmenemisprosentit tai numerot diagrammina</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Havainnollista näiden
                        typologioiden ilmenemisprosentit tai numerot tietyissa rahakätköissä
                        diagrammina</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Lataa tiettyihin rahakätköihin
                        rajoitettujen tiettyjen hakujen hakutuloksien tiedot</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">valitse
                        mitat</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">valitse
                        taulukkotyyppi</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">valitse kategoriat
                        analyysiin/valitse luokat tutkimukseen</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">valitse rahakätkö</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">valitse rahakätköt
                        vartailuun (vapaaehtoinen)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">vertaa
                        kategorioittain/vertaa luokittain</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">vertaa toisiin hakuihin
                        (vapaaehtoinen)</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Käytä pudotusvalikkoja
                        mittahaun luomiseen. Ajanjakson voi määrittää vain kerran joka
                        haulle.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">vaihtoehtoiset
                        asetukset</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Piilota/näytä
                        vaihtoehdot</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'"
                        >Kokoamisvaihtoehdot?</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Jätä tietyt koodit
                        pois</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">kumulatiivinen
                        prosentti</xsl:when>
                    <xsl:when test="$label = 'chart_bar'">pylväs (e.g. pylväsdiagrammi for
                        barchart)</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error2'">Loppumispäiväärän on oltava
                        myöhemmin kuin aloituspäivämäärän.</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Valitse luokka/valitse
                        kategoria</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Valitse yhdestä kahdeksaan
                        rahakätköä/ Valitse 1-8 rahakätköä</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Voit valita enintään 8
                        rahakätköä.</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Ainakin 1, ja enintään 30
                        rahakätköä valittavissa.</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Voit valita enintään 30
                        rahakätköä.</xsl:when>
            ...
            <xsl:when test="$lang = 'da'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Vis posterne XX til YY fra ZZ
                        resultater</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Anvend de udvalgte data og
                        visualiseringsmulighederne nedenfor til at danne en grafisk fremstilling
                        baseret på udvalgte parametre. Se instruktioner her </xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Brug dette redskab til at
                        visualisere procenter eller absolutte tal for forekomster af de følgende
                        typer </xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Brug dette redskab til at
                        gengive procenter eller antal af forekomster af mønter fra en bestemt
                        periode i skatte </xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Brug dette redskab til download
                        af CSV for søgningen og udvalgte skattefund </xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_categories'">Udvælg kategorier til
                        analyse</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Vælg skatte</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Vælg skatte til
                        sammenligning (valgfri)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Sammenlign
                        kategorier</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Sammenlign med andre
                        forespørgsler (valgfri) </xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Sammenlign
                        forespørgsler</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'"
                        >Sammenligningsforespørgsel</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Tilføj særlige
                        forespørgsler</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Særlige
                        forespørgsler</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_queries'">Tilføj
                        forespørgsler</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_query'">Filtrer
                        forespørgsel</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Fjern filter</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Anvend menyerne nedenfor
                        til at formulere forespørgsler om mål. Der kan kun defineres en periode pr.
                        forespørgsel</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Valgfrie
                        indstillinger</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Udelad
                        sikkerhedskoder</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Arranger efter periode
                        (valgfri)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Akkumuleret
                        procent</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_alert'">alarm</xsl:when>
                    <xsl:when test="$label = 'visualize_error1'">Periode og varighed
                        kræves</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Til Dato skal være senere end Fra
                        Dato.</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Vælg en kategori</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Vælg fra 1 til 8
                        skattefund</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Op til 8 skattefund kan
                        vælges</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Vælg fra 1 til 30
                        skattefund</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Op til 30 skattefund kan
                        vælges</xsl:when>
            ...
            <xsl:when test="$lang = 'hu'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'header_visualize'">Lekérdezések
                        megjelenítése</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_clear-all'">Összes feltételek
                        törlése</xsl:when>
                    <xsl:when test="$label = 'results_data-options'">Adat opciók</xsl:when>
                    <xsl:when test="$label = 'results_refine-results'">Eredmények
                        szűkítése</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-category'">Kategóriák
                        rendezése</xsl:when>
                    <xsl:when test="$label = 'results_ascending'">Növekvő</xsl:when>
                    <xsl:when test="$label = 'results_descending'">Csökkenő</xsl:when>
                    <xsl:when test="$label = 'results_result-desc'">Bejegyzések megjelenítése XX-től
                        YY-ig ZZ összes eredményből.</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Használja a kiválasztott adatokat és
                        az alábbi megjelenítési lehetőségeket, hogy létrehozz egy kiválasztott
                        paramétereken alapuló táblázatot. Itt talál útmutatót a funkció
                        használatáról</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Használja ezt a funkciót, hogy
                        meghatározza a százalékos vagy numerikus előfordulását a következő
                        tipológiáknak</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Használja ezt a funkciót a
                        kincsleleten belül az érmek százalékos vagy numerikus előfordulásának
                        meghatározására egy adott időpontban</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Használja ezt a funkciót, hogy a
                        megadott lekérdezés és a kiválasztott kincslelethez letöltsön egy
                        CSV-t</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Méretek
                        kiválasztása</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Diagram típus
                        kiválasztása</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Kategóriák kiválasztása az
                        elemzéshez</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Kincsleletek
                        kiválasztása</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Kincsleletek
                        kiválasztása összehasonlítás céljából (választható)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Összehasonlítás kategória
                        szerint</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Összehasonlítás más
                        lekérdezésekkel (választható)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Lekérdezések
                        összehasonlítása</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Lekérdezés
                        összehasonlítása</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Specifikus lekérdezések
                        hozzáadása</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Specifikus
                        lekérdezés</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_query'">Lekérdezés
                        eltávolítása</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query'">Lekérdezés hozzáadása</xsl:when>
                    <xsl:when test="$label = 'visualize_add_queries'">Lekérdezések
                        hozzáadása</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_remove_filter'">szűrő
                        eltávolítása</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Használja az alábbi
                        legördülő menüt a mérés lekérdezés megfogalmazásához. Dátumtartományok csak
                        egyszer adhatók meg egy lekérdezés során.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Választható
                        beállítások</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Lehetéségek megmutatása /
                        elrejtése</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Egymásra halmozott
                        lehetőségek</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Bizonyos kódok
                        kizárása</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Elrendezés Intervallum szerint
                        (nem kötelező)</xsl:when>
                    <xsl:when test="$label = 'visualize_interval'">Intervallum (év)</xsl:when>
                    <xsl:when test="$label = 'visualize_duration'">Időtartam</xsl:when>
                    <xsl:when test="$label = 'visualize_calculate'">Kiválasztottak
                        számolása</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Halmozott
                        százalék</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Szükség van intervallumra és
                        időtartamra.</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">A befejezési dátumnak a kezdő dátum
                        után kell lennie</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Ki kell választani egy
                        kategóriát</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Legalább 1, legfeljebb 8 kincslelet
                        választható ki</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">legfeljebb 8 kincslelet választható
                        ki</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Legalább 1, legfeljebb 30
                        kincslelet választható ki</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">legfeljebb 30 kincslelet
                        választható ki</xsl:when>
            ...
            <xsl:when test="$lang = 'el'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_examples'">Παραδείγματα του ίδιου
                        τύπου</xsl:when>
                    <xsl:when test="$label = 'results_all-terms'">Επιλογή όλων των όρων</xsl:when>
                    <xsl:when test="$label = 'results_map-results'">Αποτελέσματα επί
                        χάρτου</xsl:when>
                    <xsl:when test="$label = 'results_filters'">Φίλτρα</xsl:when>
                    <xsl:when test="$label = 'results_keyword'">Λέξη-κλειδί</xsl:when>
                    <xsl:when test="$label = 'results_clear-all'">Εκκαθάριση όλων των
                        όρων</xsl:when>
                    <xsl:when test="$label = 'results_data-options'">Επιλογές δεδομένων</xsl:when>
                    <xsl:when test="$label = 'results_refine-results'">Περιορισμός
                        αποτελεσμάτων</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-search'">Περιορισμός
                        αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'results_select'">Επιλογή από τη λίστα</xsl:when>
                    <xsl:when test="$label = 'results_sort-results'">Ταξινόμηση
                        αποτελεσμάτων</xsl:when>
                    <xsl:when test="$label = 'results_sort-category'">Ταξινόμηση
                        κατηγοριών</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Παρουσίαση αποτελεσμάτων XX έως
                        YY από συνολικά ZZ</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_measurement'">Ανάλυση μετρικών
                        δεδομένων</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Χρησιμοποιείστε την επιλογή δεδομένων
                        και τις δυνατότητες οπτικοποίησης κάτωθι για να δημιουργήσετε ένα γράφημα
                        βασισμένο σε επιλεγμένες παραμέτρους. Οδηγίες για τη χρήση αυτού του
                        εργαλείου υπάρχουν εδώ.</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Χρησιμοποιείστε αυτό το εργαλείο
                        για να οπτικοποιήσετε ποσοστά ή αριθμητικά δεδομένα για τους παρακάτω
                        τύπους</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Χρησιμοποιείστε αυτό το εργαλείο
                        για να οπτικοποιήσετε ποσοστά ή αριθμητικά δεδομένα όσον αφορά σε νομίσματα
                        μιας ιδιαίτερης χρονολόγησης εντός «θησαυρών»</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Χρησιμοποιείστε αυτό το εργαλείο
                        για να κατεβάσετε ένα αρχείο CSV (δεδομένα χωρισμένα με κόμματα) για τη
                        συγκεκριμένη αναζήτηση και τους επιλεγμένους «θησαυρούς»</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Επιλέξτε μετρικά
                        δεδομένα</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Επιλέξτε τύπο
                        γραφήματος</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Επιλέξτε κατηγορίες
                        ανάλυσης</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Επιλέξτε
                        «θησαυρούς»</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Επιλέξτε
                        «θησαυρούς» για σύγκριση (προαιρετικά)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Σύγκριση βάσει
                        κατηγορίας</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Αντιπαραβολή με άλλες
                        αναζητήσεις (προαιρετικά)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Αντιπαραβολή
                        αναζητήσεων</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Συνδυαστική
                        αναζήτηση</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Προσθήκη συνήθων
                        αναζητήσεων</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Συνήθης αναζήτηση</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_query'">Εκκαθάριση
                        αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query'">Προσθήκη αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_add_queries'">Προσθήκη
                        αναζητήσεων</xsl:when>
                    <xsl:when test="$label = 'visualize_add_new'">Προσθήκη νέας
                        αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_query'">Υποβολή αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_filter_list'">Λίστα φίλτρων
                        αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_filter_query'">Φιλτράρισμα
                        αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Εκκαθάριση φίλτρου
                        αναζήτησης</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Χρησιμοποιείστε τα κάτωθι
                        μενού για να διαμορφώσετε μια αναζήτηση μετρικών δεδομένων. Μόνο μία χρονική
                        περίοδος μπορεί να ορισθεί για κάθε αναζήτηση.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Προαιρετικές
                        ρυθμίσεις</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Απόκρυψη/εμφάνιση
                        επιλογών</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Επιλογές
                        stacking</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Εξαίρεση certainty
                        codes</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Κατάταξη βάσει χρονικού
                        διαστήματος (προαιρετικά)</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_calculate'">Υπολογισμός
                        επιλεγμένων</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Συγκεντρωτικό
                        ποσοστό</xsl:when>
                    ...
                    <xsl:when test="$label = 'chart_spline'">πολυωνυμική καμπύλη
                        γραφήματος</xsl:when>
                    <xsl:when test="$label = 'chart_areaspline'">περιοχή πολυωνυμικής καμπύλης
                        γραφήματος</xsl:when>
                    <xsl:when test="$label = 'chart_line'">γραμμή γραφήματος</xsl:when>
                    <xsl:when test="$label = 'visualize_alert'">Προσοχή!</xsl:when>
                    <xsl:when test="$label = 'visualize_error1'">Χρονικό διάστημα και διάρκεια είναι
                        απαραίτητα.</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Η καταληκτήρια χρονολογία πρέπει να
                        είναι μεταγενέστερη από την εναρκτήρια.</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Μια κατηγορία πρέπει να
                        επιλεχθεί.</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Μπορούν να επιλεχθούν ένας
                        (τουλάχιστον) έως οκτώ «θησαυροί».</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Μπορούν να επιλεχθούν μέχρι οκτώ
                        «θησαυροί».</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Μπορούν να επιλεχθούν ένας
                        (τουλάχιστον) έως τριάντα «θησαυροί».</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Μπορούν να επιλεχθούν μέχρι τριάντα
                        «θησαυροί».</xsl:when>
            ...
            <xsl:when test="$lang = 'es'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_quantitative'">Análisis
                        cuantitativo</xsl:when>
                    <xsl:when test="$label = 'display_date-analysis'">Análisis de los
                        datos</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_data-options'">Ocultar todos los
                        términos</xsl:when>
                    <xsl:when test="$label = 'results_refine-results'">Detallar
                        resultados</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-search'">Volver a definir la
                        búsqueda</xsl:when>
                    <xsl:when test="$label = 'results_select'">Seleccionar</xsl:when>
                    <xsl:when test="$label = 'results_sort-results'">Ordenar los
                        resultados</xsl:when>
                    <xsl:when test="$label = 'results_sort-category'">Ordenar por
                        categorías</xsl:when>
                    <xsl:when test="$label = 'results_ascending'">Ascendente</xsl:when>
                    <xsl:when test="$label = 'results_descending'">Descendente</xsl:when>
                    <xsl:when test="$label = 'results_result-desc'">Resultados de XX a YY sobre un
                        total de ZZ</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Utiliza las opciones para la
                        selección y visualización de los datos de debajo para crear un gráfico
                        basado en los parámetros seleccionados. Las instrucciones para utilizar esta
                        función se pueden consultar aquí.</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Utiliza esta función para
                        visualizar el porcentaje o la frecuencia numérica de esta
                        tipología.</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Utilizar esta función para
                        obtener el porcentaje o el número de veces en los que esta moneda aparece en
                        una fecha determinada en los tesoros</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Utiliza esta función para
                        descargar CSV (valores separados por comas) relativos a la consulta
                        realizada y a los tesoros seleccionados.</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Seleccionar
                        dimensiones</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Mapa tipo</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Categorías</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Seleccionar
                        tesoros</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Seleccionar tesoros
                        opcional</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Comparar
                        categoría</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Comparar
                        opcional</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Comparar</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Comparar
                        búsqueda</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Añadir búsquedas
                        personales</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_query'">Filtrar la
                        búsqueda</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Limpiar filtro</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Utiliza el menú desplegable
                        inferior para elaborar tu consulta de medida. Se puede especificar un arco
                        cronológico sólo una vez por cada consulta.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Parámetros
                        opcionales</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Ocultar Mostrar</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Juntar
                        opciones</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Excluir los
                        códigos ciertos</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Organizar por
                        intervalos</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Porcentaje
                        acumulativo</xsl:when>
                    <xsl:when test="$label = 'chart_bar'">Barra</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Se requiere intervalo y
                        período</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">La fecha final debe ser posterior a
                        la fecha inicial</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Se debe seleccionar una
                        categoría</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Se puede seleccionar un mínimo de 1
                        tesoro y un máximo de 8</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Sólo se pueden seleccionar 8
                        tesoros</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Sólo se puede seleccionar un mínimo
                        de 1 tesoro y un máximo de 30</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Se pueden seleccionar hasta 30
                        tesoros</xsl:when>
            ...
            <xsl:when test="$lang = 'bg'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'header_analyze'">Анализ на монетни
                        съкровища</xsl:when>
                    <xsl:when test="$label = 'header_visualize'">Визуализация на
                        запитванията</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_refine-results'">Ограничи
                        резултатите</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_sort-category'">Сортирай
                        категориите</xsl:when>
                    <xsl:when test="$label = 'results_ascending'">възходящо</xsl:when>
                    <xsl:when test="$label = 'results_descending'">низходящо</xsl:when>
                    <xsl:when test="$label = 'results_result-desc'">Показване на записи от ХХ до YY,
                        от общо ZZ резултата</xsl:when>
                    <xsl:when test="$label = 'results_coin'">Монета</xsl:when>
                    <xsl:when test="$label = 'results_coins'">Монети</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_desc'">Използвайте избранните данни и
                        възможностите за визуализиране отдолу, за да генерирате диаграма, базирана
                        на подбраните параметри. Инструкции за употребата на тази функция могат да
                        бъдат намерени тук</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Използвайте тази функция, за да
                        визуализирате процентите или броя на появата на следните
                        типологии</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Използвайте тази функция, за да
                        изведете процентите или броя на появата на монети с определена дата в
                        съкровищата</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Използвайте тази функция, за да
                        свалите CSV за конкретното запитване и избраните монетни
                        съкровища</xsl:when>
                    <xsl:when test="$label = 'visualize_select_measurement'">Избери
                        размери</xsl:when>
                    <xsl:when test="$label = 'visualize_chart_type'">Избери тип диаграма</xsl:when>
                    <xsl:when test="$label = 'visualize_categories'">Избери категории за
                        анализ</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards'">Избери монетни
                        съкровища</xsl:when>
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Избери монетни
                        съкровища за сравняване (по избор)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Сравняване по
                        категория</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Сравни с други запитвания
                        (по избор)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Сравни запитванията</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Запитване за
                        сравнение</xsl:when>
                    <xsl:when test="$label = 'visualize_add_custom'">Добави специализирано
                        запитване</xsl:when>
                    <xsl:when test="$label = 'visualize_custom_query'">Специализирано
                        запитване</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_filter_list'">Филтриране на
                        списъка</xsl:when>
                    <xsl:when test="$label = 'visualize_filter_query'">Филтриране на
                        запитванията</xsl:when>
                    <xsl:when test="$label = 'visualize_remove_filter'">Премахни филтъра</xsl:when>
                    <xsl:when test="$label = 'visualize_add_query_desc'">Използвайте падащото меню
                        по-долу, за да формулирате своето запитване за размерите. Може да се избере
                        само по един времеви период на запитване.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Настройки по
                        избор</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Покажи / скрий
                        опциите</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Опции за
                        комбиниране</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Изключи
                        определящите кодове</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Подреди по периоди (по
                        избор)</xsl:when>
                    ...
                    <xsl:when test="$label = 'numeric_cumulative_percentage'">Нарастващ
                        процент</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_error1'">Необходими са интервал и
                        продължителност</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">Крайната дата трябва да бъде
                        по-късна от началната дата</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">Трябва да бъде избрана
                        категория</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">Могат да избрани от (най-малко) 1
                        до (най-много) 8 съкровища</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Могат да бъдат избрани до 8
                        съкровища</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">Могат да избрани от (най-малко) 1
                        до (най-много) 30 съкровища</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Могат да бъдат избрани до 30
                        съкровища</xsl:when>
            ...
            <xsl:when test="$lang = 'he'">
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'visualize_error6'">יש לבחור לפחות מטמון אחד, אך לא
                        יותר מ-30</xsl:when>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="$label = 'display_data-download'">Data Download</xsl:when>
                    <xsl:when test="$label = 'display_quantitative'">Quantitative
                        Analysis</xsl:when>
                    ...
                    <xsl:when test="$label = 'display_subtype_desc'">Click on the link to the
                        subtype to view all examples.</xsl:when>
                    ...
                    <xsl:when test="$label = 'results_result-desc'">Displaying records XX to YY of
                        ZZ total results.</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_typological'">Typological
                        Analysis</xsl:when>
                    <xsl:when test="$label = 'visualize_measurement'">Measurement
                        Analysis</xsl:when>
                    <xsl:when test="$label = 'visualize_desc'">Use the data selection and
                        visualization options below to generate a chart based on selected
                        parameters. Instructions for using this feature can be found here</xsl:when>
                    <xsl:when test="$label = 'visualize_type_desc'">Use this feature to visualize
                        percentages or numeric occurrences of the following typologies</xsl:when>
                    <xsl:when test="$label = 'visualize_date_desc'">Use this feature to render
                        percentages or numeric occurrences of coins of a particular date within
                        hoards</xsl:when>
                    <xsl:when test="$label = 'visualize_csv_desc'">Use this feature to download a
                    ...
                    <xsl:when test="$label = 'visualize_select_hoards_optional'">Select Hoards to
                        Compare (optional)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_category'">Compare by
                        Category</xsl:when>
                    <xsl:when test="$label = 'visualize_compare_optional'">Compare to other Queries
                        (optional)</xsl:when>
                    <xsl:when test="$label = 'visualize_compare'">Compare Queries</xsl:when>
                    <xsl:when test="$label = 'visualize_comparison_query'">Comparison
                        Query</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_add_query_desc'">Use the drop-down menus
                        below to formulate your measurement query. A date range can only be
                        specified once per query.</xsl:when>
                    <xsl:when test="$label = 'visualize_optional_settings'">Optional
                        Settings</xsl:when>
                    <xsl:when test="$label = 'visualize_hide-show'">Hide/Show Options</xsl:when>
                    <xsl:when test="$label = 'visualize_stacking_options'">Stacking
                        Options</xsl:when>
                    <xsl:when test="$label = 'visualize_exclude_certainty_codes'">Exclude Certainty
                        Codes</xsl:when>
                    <xsl:when test="$label = 'visualize_arrange'">Arrange by Interval
                        (optional)</xsl:when>
                    ...
                    <xsl:when test="$label = 'visualize_alert'">Alert</xsl:when>
                    <xsl:when test="$label = 'visualize_error1'">Interval and duration are
                        required.</xsl:when>
                    <xsl:when test="$label = 'visualize_error2'">To Date must be later than From
                        Date.</xsl:when>
                    <xsl:when test="$label = 'visualize_error3'">A category must be
                        selected.</xsl:when>
                    <xsl:when test="$label = 'visualize_error4'">At least 1 and up to 8 hoards may
                        be selected.</xsl:when>
                    <xsl:when test="$label = 'visualize_error5'">Up to 8 hoards may be
                        selected.</xsl:when>
                    <xsl:when test="$label = 'visualize_error6'">At least 1 and up to 30 hoards may
                        be selected.</xsl:when>
                    <xsl:when test="$label = 'visualize_error7'">Up to 30 hoards may be
                        selected.</xsl:when>
    ...
    <xsl:function name="numishare:normalizeProperty">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('nmo:has', upper-case(substring($name, 1, 1)), substring($name, 2))"
                />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="numishare:hoardContentsDescription">
                ...
                <xsl:for-each select="$contents/descendant::nuds:typeDesc">
                    <xsl:choose>
                        <xsl:when test="string(@xlink:href)">
                            ...
                            <xsl:apply-templates
                                select="$nudsGroup//object[@xlink:href = $href]/descendant::nuds:typeDesc/nuds:denomination"
                                        ...
                                        <xsl:otherwise>
                                            <xsl:choose>
                                                ...
                                                <xsl:when test="ancestor::nh:coinGrp/@minCount">
                                                  <xsl:value-of
                                                  select="ancestor::nh:coinGrp/@minCount"/>
                                                </xsl:when>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:choose>
                                                ...
                                                <xsl:when test="ancestor::nh:coinGrp/@maxCount">
                                                  <xsl:value-of
                                                  select="ancestor::nh:coinGrp/@maxCount"/>
                                                </xsl:when>
                                            </xsl:choose>
                                        </xsl:otherwise>
                                    </xsl:choose>
        ...
        <xsl:variable name="denominations" as="element()*">
            <denominations>
                <xsl:for-each
                    select="distinct-values($total-counts//*[local-name() = 'name' and string-length(normalize-space(.)) &gt; 0])">
                    <xsl:variable name="name" select="."/>
                    <name>
                        <xsl:attribute name="minCount">
                            <xsl:value-of
                                select="sum($total-counts//*[local-name() = 'name'][. = $name]/@minCount)"
                            />
                        </xsl:attribute>
                        <xsl:attribute name="maxCount">
                            <xsl:value-of
                                select="sum($total-counts//*[local-name() = 'name'][. = $name]/@maxCount)"
                            />
                        </xsl:attribute>
        ...
        <xsl:variable name="description">
            <xsl:apply-templates select="$denominations//*[local-name() = 'name']"
                mode="hoardContentsDescription">
        ...
        <xsl:variable name="label">
            <xsl:choose>
                <xsl:when test="contains($href, 'nomisma.org')">
                    <xsl:value-of
                        select="numishare:getNomismaLabel($rdf/*[@rdf:about = $href], $lang)"/>
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:when test="string($source)">
                <xsl:choose>
                    <xsl:when test="$contents//nh:coin[nuds:typeDesc[@xlink:href = $source]]">
                        <name>
                            <xsl:attribute name="minCount"
                                select="count($contents//nh:coin/nuds:typeDesc[@xlink:href = $source])"/>
                            <xsl:attribute name="maxCount"
                                select="count($contents//nh:coin/nuds:typeDesc[@xlink:href = $source])"/>
                    ...
                    <xsl:when test="$contents//nh:coinGrp[nuds:typeDesc[@xlink:href = $source]]">
                        ...
                        <name>
                            <xsl:attribute name="minCount"
                                select="sum($contents//nh:coinGrp[nuds:typeDesc[@xlink:href = $source]]/@count)"/>
                            <xsl:attribute name="maxCount"
                                select="sum($contents//nh:coinGrp[nuds:typeDesc[@xlink:href = $source]]/@count)"/>
                            <xsl:value-of select="$label"/>
                        </name>
    ...
    <xsl:function name="numishare:getLabelforRDF">
        ...
        <xsl:choose>
            <xsl:when test="$lang = 'en'">
                <xsl:choose>
                    <xsl:when test="$element = 'crm:P106_is_composed_of'">Constituent
                        Letters</xsl:when>
        ...
        <xsl:variable name="xml-url">
            <xsl:choose>
                <xsl:when test="matches($uri, '^https://rpc\.ashmus\.ox\.ac\.uk')">
                    ...
                    <xsl:value-of
                        select="concat('https://rpc.ashmus.ox.ac.uk/id/rpc-', $pieces[5], '-', $pieces[6], '.xml')"
                    />
                </xsl:when>
                <xsl:otherwise>
                            ...
                            <xsl:when test="contains($piece, ':')">
                                <xsl:choose>
                                    ...
                                    <xsl:when test="substring($piece, 1, 1) = '('">
                                        ...
                                        <xsl:variable name="or_frags"
                                            select="tokenize(replace(replace($piece, '\(', ''), '\)', ''), ' OR ')"/>
                                        <xsl:variable name="field"
                                            select="substring-before($or_frags[1], ':')"/>

                                        <xsl:value-of
                                            select="numishare:normalize_fields($field, $lang)"/>
                                        <xsl:text>: </xsl:text>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:value-of
                                            select="numishare:normalize_fields(substring-before($piece, ':'), $lang)"/>
                                        <xsl:text>: </xsl:text>
                                        <xsl:value-of
                                            select="replace(substring-after($piece, ':'), '&#x022;', '')"
                                        />
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
            ...
            <xsl:choose>
                <xsl:when test="contains(., '?prop')">
                    <xsl:analyze-string select="." regex="\?prop\s(nm:.*)">
                        <xsl:matching-substring>
                            <xsl:value-of
                                select="concat(numishare:regularize_node('authority', $lang), '/', numishare:regularize_node('issuer', $lang))"/>
                            <xsl:text>: </xsl:text>
                ...
                <xsl:otherwise>
                    <xsl:analyze-string select="." regex="nmo:has([A-Za-z]+)\s(nm:.*)">
                        <xsl:matching-substring>
                            <xsl:value-of
                                select="numishare:regularize_node(lower-case(regex-group(1)), $lang)"/>
                            <xsl:text>: </xsl:text>
    ...
    <xsl:function name="nomisma:getLabel">
        ...
        <xsl:variable name="service"
            select="concat('http://nomisma.org/apis/getLabel?uri=', $uri, '&amp;lang=', $lang)"/>
        ...
        <xsl:choose>
            <xsl:when test="substring($date, 1, 1) = '-'">
                <xsl:choose>
                    ...
                    <xsl:when test="$date castable as xs:date">
                        <xsl:value-of
                            select="concat(xs:date($date) + xs:dayTimeDuration('P365DT0M'), $time)"
                        />
                    </xsl:when>
                    <xsl:when test="$date castable as xs:dateTime">
    ...
    <xsl:function name="numishare:resolveUriToCurie">
        ...
        <xsl:choose>
            <xsl:when test="$namespaces//namespace[contains($uri, @uri)]/@prefix">
                <xsl:choose>
                    <xsl:when
                        test="$concept/skos:*[contains(local-name(), 'Match')][contains(@rdf:resource, 'http://vocab.getty.edu')]">
